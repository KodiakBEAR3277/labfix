<script setup>
// Pages/IT/Assignments/Show.vue
// Path: resources/js/Pages/IT/Assignments/Show.vue
// Mirrors: resources/views/it/assignments/show.blade.php
...
function startWorking() {
  startForm.post(`/it/assignments/${props.ticket.id}`)
}

function markResolved() {
  resolveForm.post(`/it/assignments/${props.ticket.id}`)
}

// ── Computed helpers from transactions ────────────────────────────────────────
const assignedTransaction = computed(() =>
  props.ticket.transactions?.find(tx => tx.action === 'assigned') ?? null
)

const resolvedTransaction = computed(() =>
  props.ticket.transactions?.find(tx => tx.action === 'status_changed' && tx.new_value === 'resolved') ?? null
)
...
<template>
  <AppLayout>
    ...
    <div id="it-ticket-detail" class="container">
      <Link href="/it/assignments" class="back-btn">← Back to My Assignments</Link>
      ...
      <div class="ticket-header">
        <div class="header-top">
          ...
          <div style="display:flex;gap:1rem;align-items:center;">
            ...
            <Link :href="`/it/assignments/${ticket.id}/edit`" class="btn btn-primary">Update Status</Link>
          </div>
        </div>
        ...
        <div class="ticket-meta-grid">
          <div class="meta-item">
            ...
            <div class="meta-value">{{ ticket.reporter?.first_name }} {{ ticket.reporter?.last_name }}</div>
          </div>
          <div class="meta-item">
          ...
          <div class="meta-item">
            ...
            <div class="meta-value">{{ ticket.category.charAt(0).toUpperCase() + ticket.category.slice(1) }}</div>
          </div>
          <div class="meta-item">
          ...
          <div class="meta-item">
            ...
            <div class="meta-value">{{ assignedTransaction ? diffForHumans(assignedTransaction.created_at) : 'Just now' }}</div>
          </div>
          <div class="meta-item">
            <div class="meta-label">Last Updated</div>
            <div class="meta-value">{{ diffForHumans(ticket.updated_at) }}</div>
          </div>
        </div>
      </div>
          ...
          <div class="card">
            ...
            <div class="info-list">
              ...
              <div class="info-item">
                ...
                <div class="info-value">{{ ticket.equipment?.type?.charAt(0).toUpperCase() + ticket.equipment?.type?.slice(1) }}</div>
              </div>
              <div class="info-item">
                ...
                <div v-for="tx in ticket.transactions" :key="tx.id" class="timeline-item">
                  ...
                  <div class="timeline-content">
                    ...
                    <div style="margin-top:0.25rem;font-size:0.85rem;color:#9ca3af;">
                      by {{ tx.user?.first_name }} {{ tx.user?.last_name }}
                    </div>
                  </div>
                </div>
              </template>
        ...
        <div class="sidebar">
          ...
          <div class="card">
            ...
            <div class="action-buttons" style="flex-direction:column;">
              <Link :href="`/it/assignments/${ticket.id}/edit`" class="btn btn-primary">Update Status</Link>
              <button
                v-if="ticket.status !== 'in-progress' && ticket.status !== 'resolved'"
                type="button"
                class="btn btn-secondary"
                :disabled="startForm.processing"
                @click="startWorking"
              >
                {{ startForm.processing ? 'Updating…' : 'Start Working' }}
              </button>
              <button
          ...
          <div class="card">
            ...
            <div class="info-list">
              ...
              <div v-if="resolvedTransaction" class="info-item">
          ...
          >
            ...
            <p v-if="ticket.status === 'assigned'" style="color:#d1d5db;font-size:0.9rem;line-height:1.6;">
              Start working on this ticket to let the reporter know you're actively resolving their issue.
            </p>
            <p v-else-if="ticket.status === 'in-progress'" style="color:#d1d5db;font-size:0.9rem;line-height:1.6;">
