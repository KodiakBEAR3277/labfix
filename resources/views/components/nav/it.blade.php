<nav>
    ...
    <div class="nav-menu">
        ...
        <a href="{{ route('it.assignments.index') }}" class="nav-link {{ request()->routeIs('it.assignments.*') ? 'active' : '' }}">My Assignments</a>
        <a href="{{ route('it.knowledge-base.index') }}" class="nav-link {{ request()->routeIs('it.knowledge-base.*') ? 'active' : '' }}">Knowledge Base</a>
        <a href="{{ route('profile.show') }}" class="user-profile">
            ...
            <span>
                {{ auth()->user()->first_name ?? 'IT' }} {{ auth()->user()->last_name ?? 'Support' }}
                @if(auth()->user()->isAdmin())
                    <span style="font-size: 0.7rem; color: #ef4444;">(Admin)</span>
                @endif
            </span>
        </a>
    </div>
